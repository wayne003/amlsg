...
for ( i in 1:b)
{
    ...
    CPSSfit <- CoxCPSSInteractions(X=as.matrix(trainX),surv=os.train,scope=which(groups %in% scope),mc.cores=ncore)
...
### AUC
    ...
    sisfit <- SIS(x=as.matrix(trainX),y=os.train,family='cox',penalty='lasso',tune='bic',iter=FALSE)
