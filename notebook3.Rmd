...
for ( i in 1:b)
{
    ...
    CPSSfit <- CoxCPSSInteractions(X=trainX.df,surv=os.train,scope=which(groups %in% scope),mc.cores=3)
...
### AUC
    ...
    prderCPSS[i] <- predErr(surv.rsp.cpss,surv.rsp.new.cpss,lp=lp.cpss, lpnew=lpnew.cpss,times=times)$ierror
    prderSIS[i] <- predErr(surv.rsp.sis,surv.rsp.new.sis,lp=lp.sis, lpnew=lpnew.sis,times=times)$ierror
}
